  ...
  const onsocket = function (sock) {
    ...
    if (sock.connecting) {
      ...
      if (options.connectTimeout) {
        debug(
          'makeRequest(): setting connectTimeout timer for %dms',
  ...
  const tryable = async function () {
    try {
      ...
      if (error.code === 'ECONNRESET') {
        debug('retryable(): connection reset error');
        if (options.resetRetries < options.maxResetRetry) {
          debug('retryable(): retrying');
          options.resetRetries++;
          return tryable();
        }
      }
      throw error;
